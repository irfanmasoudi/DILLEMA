...
captions_lim_folder = pathlib.Path("/home/ubuntu/lim25-imagenet-captions/")
counterfactuals_folder = pathlib.Path("/home/ubuntu/lim25-imagenet-counterfactuals/")
word_lists_folder = pathlib.Path("/home/ubuntu/lim25-imagenet-word_lists/")
alternatives_folder = pathlib.Path("/home/ubuntu/lim25-imagenet-alternatives/")
applied_alternatives_folder = pathlib.Path("/home/ubuntu/lim25-imagenet-applied_alternatives/")
...
#percentage = int((35/100)*len(dataset))
...
for i, elem in tqdm(enumerate(dataset)):
    ...
    if img_file_name == 'n01753488_102.JPEG':
        print('skipped')
        continue
    ...
    if img_dir != last_dir:
        ...
        if count > 24:
            ...
            last_dir = img_dir
            continue
        caption_file_lim = captions_lim_folder.joinpath(img_dir).joinpath(img_file_name.replace('JPEG', 'txt'))
        ...
        with open(caption_file, 'r') as f:
            ...
            try: 
                ...
                print('error, then skipped')
                continue

            if len(counterfactual) > 0:
                count += 1
                print(counterfactual_file)
                caption_file_lim.parent.mkdir(parents=True, exist_ok=True)
                with open(caption_file_lim, 'w') as f:
                    f.write(caption)
